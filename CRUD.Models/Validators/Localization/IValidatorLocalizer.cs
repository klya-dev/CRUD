...
public interface IValidatorLocalizer
{
    ...
    /// Как пользоваться
    /// <code>
    /// RuleFor(x => x.Firstname).Firstname().WithMessage(localizer[ValidatorLocalizerConstants.TestParams, 10, "100"]);
    /// </code>
    /// </example>
    /// </remarks>
    /// <exception cref="ArgumentException">Если ключ не найден.</exception>
    /// <param name="key">Ключ из <see cref="ValidatorLocalizerConstants"/>, который содержит "$A$, $B$...".</param>
    /// <param name="args">Список элементов для замены.</param>
    /// <returns>Локализированная строка с заменёнными аргументами.</returns>
    string ReplaceParams(string key, IEnumerable<string> args);
}