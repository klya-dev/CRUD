...
public sealed class CreateUserDtoValidatorTest
{
    ...
    public async Task ValidateAsync_CorrectData_ReturnsIsValid(string firstname, string username, string password, string languageCode, string email, string phoneNumber)
    {
        ...
        // Act
        var result = await _validator.ValidateAsync(createUserDto, cancellation: TestContext.Current.CancellationToken);
    ...
    public async Task ValidateAsync_NullObject_ThrowsInvalidOperationException()
    {
        ...
        Func<Task> a = async () =>
        {
            await _validator.ValidateAsync(createUserDto, TestContext.Current.CancellationToken);
        };
