...
public sealed class ImageSingnatureChecker : IImageSingnatureChecker
{
    public (bool IsValid, string FileExtension, string ContentType) IsFileValid(Stream stream)
    {
        ...
        {
            var extension = kvp.Key.Extension;
            var contentType = kvp.Key.ContentType;
            var sigs = kvp.Value;
            foreach (var sig in sigs)
                if (headerSpan.Length >= sig.Length && headerSpan.Slice(0, sig.Length).SequenceEqual(sig))
                    return (true, extension, contentType);
        }

        return (false, null!, null!);
    }

    private static readonly IReadOnlyDictionary<(string Extension, string ContentType), byte[][]> _fileSignatures = new Dictionary<(string Extension, string ContentType), byte[][]>()
    {
        ...
        { ("jpeg2000", "image/jpeg"), new byte[][] { [0x00, 0x00, 0x00, 0x0C, 0x6A, 0x50, 0x20, 0x20, 0x0D, 0x0A, 0x87, 0x0A] } },
        { ("jpg", "image/jpg"), new byte[][]
            {
                [0xFF, 0xD8, 0xFF, 0xE0],
                [0xFF, 0xD8, 0xFF, 0xE1],
                [0xFF, 0xD8, 0xFF, 0xE8],
