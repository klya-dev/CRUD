...
public sealed class PublicationsSystemTest : IClassFixture<TestWebApplicationFactory>
{
    ...
    public async Task Get_Paginated_ReturnsPaginatedListDto()
    {
        ...
        var publication = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow, "Первые шаги1", "Работа с REST API и отправка первых запросов.", null, TestContext.Current.CancellationToken);
        var publication2 = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow.AddDays(-1), "Ошибки", "Коды ответов сервера: 200, 400, 401 и 500.", DateTime.UtcNow, TestContext.Current.CancellationToken);
        ...
        var publication4 = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow.AddDays(-3), "Тестирование", "Отклик бэкенда под нагрузкой.", DateTime.UtcNow.AddDays(-1), TestContext.Current.CancellationToken);
        ...
        var mustResult = new PaginatedListDto<PublicationDto>()
        {
            ...
            TotalPages = 3,
            SearchString = null,
            SortBy = SortByVariables.date_desc,
    ...
    public async Task Get_Paginated_SearchStringLengthNotValid_ReturnsPaginatedListDtoWithEmptyCollection()
    {
        ...
        // Добавляем публикации в базу
        var publication = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow, "Первые шаги", searchString, null, TestContext.Current.CancellationToken);
        var publication2 = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow.AddDays(-1), "Ошибки", "Коды ответов сервера: 200, 400, 401 и 500.", DateTime.UtcNow, TestContext.Current.CancellationToken);
        var publication3 = await DI.CreatePublicationAsync(_db, null, DateTime.UtcNow.AddDays(-2), "Анонимные заметки", "Без привязки к автору.", null, TestContext.Current.CancellationToken);
        var publication4 = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow.AddDays(-3), "Тестирование", "Отклик бэкенда под нагрузкой.", DateTime.UtcNow.AddDays(-1), TestContext.Current.CancellationToken);
        var publication5 = await DI.CreatePublicationAsync(_db, user.Id, DateTime.UtcNow.AddDays(-4), "Заключение", "Финальная тестовая статья.", DateTime.UtcNow.AddDays(-2), TestContext.Current.CancellationToken);
        ...
        var mustResult = new PaginatedListDto<PublicationDto>()
        {
            ...
            PageSize = pageSize,
            TotalPages = 1,
            SearchString = searchString[0..validLength], // Отправляли 51 символ, стало 50 символов - очистилась до валидного состояния
            SortBy = SortByVariables.date_desc,
            HasPreviousPage = false,
            HasNextPage = false
        };
    ...
    public async Task Get_Paginated_SortBy_ReturnsPaginatedListDto()
    {
        ...
        var publication3 = await DI.CreatePublicationAsync(_db, user2.Id, DateTime.UtcNow.AddDays(-2), "Анонимные заметки", "Без привязки к автору.", null, TestContext.Current.CancellationToken);
        ...
        var publication5 = await DI.CreatePublicationAsync(_db, null, DateTime.UtcNow.AddDays(-4), "Заключение", "Финальная тестовая статья.", DateTime.UtcNow.AddDays(-2), TestContext.Current.CancellationToken);
